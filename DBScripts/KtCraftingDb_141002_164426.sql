...
create table `user_group` (
   ...
   `group_oid`  integer not null,
  primary key (`user_worker_id`, `group_oid`)
);
alter table `user_group`   add index fk_user_group_user (`user_worker_id`), add constraint fk_user_group_user foreign key (`user_worker_id`) references `user` (`worker_worker_id`);
...
alter table `task_worker`   add index fk_task_worker_worker (`worker_worker_id`), add constraint fk_task_worker_worker foreign key (`worker_worker_id`) references `worker` (`worker_id`);


-- Worker_Skills [rel2]
alter table `skills`  add column  `worker_worker_id`  integer;
...
-- GEN FK: User --> Worker
alter table `user`   add index fk_user_worker (`worker_worker_id`), add constraint fk_user_worker foreign key (`worker_worker_id`) references `worker` (`worker_id`);
